...
window.UTIL.Date.getTimeTo__YYYY_MM_DD_berfore_day = function(date,day){
...
window.COMPONENT.gnb = function(){
	...
	var urls = {
		gnb_00 : `/html/realTimeTrade.html`,
		gnb_01 : `/html/trader.html?date=20220502`,
		gnb_02 : `/html/foreignTrader.html?date=20220502`,
		gnb_03 : `/html/foreignTraderTotal.html?date=20220502`,
		gnb_04 : `/html/wics.html`,
		gnb_05 : `/html/favStock.html?date=20220430`,
		gnb_06 : `/html/marketIndex.html`,
	};
...
window.COMPONENT.renderTopRankVolume = function(){
	...
	var status = {
		...
		"RISING" : {	color:"red", symbol : "▲",},
		"FALLING" : {	color:"blue", symbol : "▼",},
		"UPPER_LIMIT" : {	color:"red", symbol : `<i class="arrow up icon red"></i>`,},
	}
	...
	option = {
		...
		series: [
			...
			let arr = [ "{title|" + params.name + "}",
				...
				"{b|거래량갭 : " + echarts.format.addCommas( params.data.amtGap ) + "}",
	...
	<div class="sixteen wide column" style="">
		<div class="ui grid">
			...
			<div class="six wide column" style="">
				<div class="image" style="min-height:230px;">
					<div claa="image"><img src="https://ssl.pstatic.net/imgfinance/chart/item/area/week/${cd}.png?sidcode=${Date.now()}"  style="width:-webkit-fill-available;"></div>
				</div>
			</div>
...
window.COMPONENT.renderCandleChartByCdnoneAcg = function(data){
	...
	window.charts.renderCandleChartByCdnoneAcg.setOption(
	(option = {
	  animation: false,
	  legend: { top: 10, left: 'left',data: ['일봉', 'MA5', 'MA20', 'MA60', 'MA120' ]  },
	  tooltip: {
		trigger: 'axis',axisPointer: { type: 'cross' }, borderWidth: 1,	borderColor: '#ccc', padding: 10, textStyle: { color: '#000'  }, 
		position: function (pos, params, el, elRect, size) {
		  const obj = { top: 10  };
	  ...
	  grid: [
		{ left: '4%', right: '2%', height: '55%' },
		{ left: '4%', right: '2%', top: '65%', height: '30%'   }
	  ],
	  xAxis: [
		{
		  type: 'category',
	  ...
	  yAxis: [
		...
		{ scale: true, gridIndex: 1,splitNumber: 2,axisLabel: { show: false,fontSize : 9 },axisLine: { show: false },axisTick: { show: false },splitLine: { show: false }},
		{ scale: true, type:"value",splitLine: { show: false },axisLine: { show: false }, axisLabel : { fontSize : 9 } },
	  ],
	  //dataZoom: [{ type: 'inside', xAxisIndex: [0, 1], start: 98, end: 100 }, { show: true, xAxisIndex: [0, 1], type: 'slider', top: '85%', start: 98, end: 100 } ],
	  series: [
		{
	...
	if (option && typeof option === 'object') {
		window.charts.renderCandleChartByCdnoneAcg.setOption(option);
	}
}
...
//	var data = splitData(rawData);
	...
	(option = {
	title : { text : info.name + " ( " + info.cd + " )"  , subtext : "주가대비 순매수 추이" , left : 'center',},
	  animation: true,
	  //legend: { top: 10, left: 'left',data: ['일봉', 'MA5', 'MA20', 'MA60', 'MA120', "개인", "외국인", "기관"]  },
	  tooltip: {
		trigger: 'axis',axisPointer: { type: 'cross' }, borderWidth: 1,	borderColor: '#ccc', padding: 10, textStyle: { color: '#000'  }, 
...
window.COMPONENT.getTrdData_buy = function( date, sort, cbFunction ){
	...
	var xhr = new XMLHttpRequest();
    xhr.open("GET" , encodeURI(`http://112.144.208.118:8888/getTrdData_buy?date=${date}&sort=${sort}` ) , true);
    xhr.onreadystatechange = function() {
...
window.COMPONENT.getTrdData_sell = function( date, sort, cbFunction ){

	date = date || window.url.params.date

	var xhr = new XMLHttpRequest();
    xhr.open("GET" , encodeURI(`http://112.144.208.118:8888/getTrdData_sell?date=${date}&sort=${sort}` ) , true);
    xhr.onreadystatechange = function() {
...
window.COMPONENT.renderAgcDailyTreemap_buy = function( d, sort ){
	...
	var domId = "renderAgcDailyTreemap_buy";
	
	var dom = document.getElementById( domId );
	if( !window.charts ) window.charts = {}
	if( !window.charts.renderAgcDailyTreemap_buy )
	{
		window.charts.renderAgcDailyTreemap_buy = echarts.init(dom);
		window.charts.renderAgcDailyTreemap_buy.isEvent = 0;
	}
...
window.COMPONENT.renderAgcDailyTreemap_sell = function( d, sort ){
	...
	var domId = "renderAgcDailyTreemap_sell";
	
	var dom = document.getElementById( domId );
	if( !window.charts ) window.charts = {}
	if( !window.charts.renderAgcDailyTreemap_sell )
	{
		window.charts.renderAgcDailyTreemap_sell = echarts.init(dom);
		window.charts.renderAgcDailyTreemap_sell.isEvent = 0;
	}
